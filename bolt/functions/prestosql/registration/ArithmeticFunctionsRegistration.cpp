...
void registerSimpleFunctions(const std::string& prefix) {
  ...
  registerFunction<
      bytedance::bolt::functions::sparksql::LogFunction,
      double,
      double,
      double>({prefix + "log"});
  ...
  // reuse pmod implementation in sparksql
  registerBinaryIntegral<bytedance::bolt::functions::sparksql::PModIntFunction>(
      {prefix + "pmod"});
  /*registerBinaryFloatingPoint<
      bytedance::bolt::functions::sparksql::PModFunction>(
      {prefix + "pmod"});*/
